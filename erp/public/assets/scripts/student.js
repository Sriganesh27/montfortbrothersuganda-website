...
window.loadStudentList = function(e) {
    ...
    if (!hasFilter) { container.innerHTML = '<div style="text-align:center; padding: 60px 20px; color: #777;"><i class="fa fa-search" style="font-size: 3rem; color: #ddd; margin-bottom: 15px;"></i><p>Please select a filter.</p></div>'; return; }
...
window.editQuickRow = function(id) {
    ...
    row.querySelector('.action-cell').innerHTML = `<div style="display:flex; gap:4px;"><button class="btn-action btn-success" onclick="confirmAction('quick_save', ${id})" style="background:var(--success-color); color:white; padding:4px 8px;"><i class="fa fa-save"></i></button><button class="btn-action btn-gray" onclick="confirmAction('quick_cancel', ${id})" style="background:#888; color:white; padding:4px 8px;"><i class="fa fa-times"></i></button></div>`;
};
...
window.cancelQuickRow = function(id) {
    ...
    row.querySelector('.action-cell').innerHTML = `<button class="btn-action view-btn" onclick="editQuickRow(${data.AdmissionNo})"><i class="fa fa-pencil"></i> Edit</button>`;
};
...
window.editAcademicRow = function(id) {
    ...
    row.querySelector('.action-cell').innerHTML = `<div style="display:flex; gap:4px;"><button class="btn-action btn-success" onclick="confirmAction('academic_save', ${id})" title="Save"><i class="fa fa-save"></i></button><button class="btn-action btn-gray" onclick="confirmAction('academic_cancel', ${id})" title="Cancel"><i class="fa fa-times"></i></button></div>`;
};
...
window.cancelAcademicRow = function(id) {
    ...
    row.querySelector('.action-cell').innerHTML = `<button class="btn-action view-btn" onclick="editAcademicRow(${id})"><i class="fa fa-pencil"></i> Edit</button>`;
};
...
window.saveAcademicRow = function(id) {
    ...
    }).catch(err => { console.error(err); window.showCustomAlert('error', 'Error', 'Connection failed.'); row.querySelector('.action-cell').innerHTML = `<button class="btn-action view-btn" onclick="editAcademicRow(${id})"><i class="fa fa-pencil"></i> Edit</button>`; });
};
...
window.handleAdmission = function(e) {
    e.preventDefault(); const form = e.target; const formData = new FormData(form); const btn = form.querySelector('button[type="submit"]'); if(btn) { btn.disabled = true; btn.textContent = "Processing..."; }
    fetch(`${getApiPrefix()}api/students/admission.php`, { method: 'POST', body: formData }).then(r => r.json()).then(data => {
...
// =========================================================================
window.autoFillAdmissionForm = function() {
    const urlParams = new URLSearchParams(window.location.search);
    const appId = urlParams.get('prefill_app_id');
    ...
    if (appId && window.location.hash === '#admission') {
        ...
        fetch(`${getApiPrefix()}api/manage_applications?action=fetch_single&app_id=${appId}`)
        .then(r => r.json())
        .then(d => {
            if (d.success) {
                const app = d.data;
                ...
                if (termSelect && app.term) {
                    ...
                    Array.from(termSelect.options).forEach(opt => {
                        if (opt.value.toLowerCase() === cleanTerm.toLowerCase() || opt.value.toLowerCase() === app.term.toLowerCase()) {
                ...
                const appLevel = app.level || app.applied_level || '';
                ...
                if (app.prev_marks_doc && app.prev_marks_doc.trim() !== '') {
                    ...
                    if(docLink) { docLink.href = docUrl; docLink.style.display = 'block'; }
...
function performGlobalSearch(query, container) {
    container.innerHTML = `<div class="search-loading-state"><i class="fa fa-spinner fa-spin search-loading-icon"></i></div>`;
...
function fetchMigrationStudents() {
    ...
    if (tableBody) tableBody.innerHTML = '<tr><td colspan="7" style="text-align:center; padding:20px;"><i class="fa fa-spinner fa-spin"></i> Loading students...</td></tr>';
...
window.loadPhotoEditData = function(page) {
    ...
    tbody.innerHTML = '<tr><td colspan="7" class="text-center" style="padding:20px;"><i class="fa fa-spinner fa-spin"></i> Loading Photos...</td></tr>';
...
window.cancelPhotoRow = function(id) {
    ...
    row.querySelector('.action-cell').innerHTML = `<button class="btn-action view-btn" onclick="editPhotoRow(${id})"><i class="fa fa-pencil"></i> Change</button>`;
};
...
window.executePhotoUpload = function(id) {
    ...
    actionCell.innerHTML = '<i class="fa fa-spinner fa-spin" style="color:var(--primary-color);"></i> Saving...';
...
window.loadAccountsData = function(page) {
    ...
    tbody.innerHTML = '<tr><td colspan="8" style="text-align:center; padding:30px;"><i class="fa fa-spinner fa-spin"></i> Loading...</td></tr>';
    ...
    data.forEach(row => {
        ...
        const passDisplay = passVal ? `
            <div class="pwd-wrapper">
                ...
                <i class="fa fa-eye pwd-toggle" onclick="togglePwdVisibility('pwd-${row.AdmissionNo}')" title="Show/Hide"></i>
           ...
           <td>${row.Class} / ${row.Stream || '-'}</td>
            ...
            <td>
                <button class="btn-action btn-sm" style="background:var(--primary-color); color:#fff; border:none; padding:4px 8px; border-radius:4px; cursor:pointer;" onclick="promptPasswordChange(${row.AdmissionNo}, '${safeName}', '${safeDetails}')">
                    <i class="fa fa-pencil"></i> Edit Pwd
                </button>
            </td>
            <td>${toggleSwitch}</td>
        `;
...
window.loadAlumniDebtsData = function(page) {
    ...
    tbody.innerHTML = '<tr><td colspan="9" style="text-align:center; padding:30px;"><i class="fa fa-spinner fa-spin"></i> Loading...</td></tr>';
    ...
    if (level === 'PLE') {
        return `
            <tr>
                ...
                <td>${row.aggregate || ''}</td>
                <td>${row.division || ''}</td>
                <td><button onclick="deleteMark(${row.id})" class="btn-delete"><i class="fa fa-trash"></i></button></td>
            </tr>`;
    } else {

...
<head>
    <meta charset="UTF-8">
    <title>Application Receipt - <?php echo displayField($data['ref_number'] ?? ''); ?></title>
...
<body>
    ...
    <div class="action-bar no-print">
        <a href="<?php echo getBaseUrl(); ?>/apply" class="btn-view-doc">Back to Portal</a>
    ...
    <div class="document-container">
        ...
        <div class="header-section">
            ...
            <div class="header-text">
                ...
                <h2>Student Application Receipt</h2>
        ...
        <div class="meta-info">
            ...
            <?php 
                $schol = trim($data['scholarship_status'] ?? '');
                if(!empty($schol) && strtolower($schol) !== 'none'): 
            ?>
                <div>Scholarship: <span style="color:#d32f2f; font-weight:bold;"><?php echo htmlspecialchars($schol); ?></span></div>
            <?php endif; ?>

            <div>Status: 
                <?php 
                    $stat = trim($data['status'] ?? 'Pending');
                    $color = '#b8860b'; 
                    $statLower = strtolower($stat);
                    if($statLower === 'admitted' || $statLower === 'selected' || $statLower === 'shortlisted') { $color = '#2f855a'; } 
                    elseif($statLower === 'rejected') { $color = '#d32f2f'; }
                ?>
                <span style="color:<?php echo $color; ?>; font-weight:bold; text-transform:uppercase;">
            ...
            <div style="display: grid; grid-template-columns: 3fr 1fr;">
                <div>
                    ...
                    <div class="grid-row">
                        ...
                        <div class="grid-item no-bottom"><span class="label">Applied Class & Level</span><span class="value"><?php echo displayField($data['applied_class'] ?? '') . ' ' . (!empty($data['class_code']) ? '['.displayField($data['class_code']).']' : '') . ' (' . displayField($data['level'] ?? '') . ')'; ?></span></div>
                    </div>
                </div>
                <div class="photo-area">
                    ...
                    <?php else: ?>
                        <div class="no-photo">Photo Received</div>
                    <?php endif; ?>
                </div>
            </div>
        </div>
        ...
        <div class="footer">
            Official Application Receipt - Generated on <?php echo date('F j, Y - H:i'); ?>. Keep this reference number for future inquiries.
        </div>
    </div>
